...
//        logger.info("dataSampleManager.hostList {}", dataSampleManager.hostList);
            ...
            if (!tempObject.getBoolean("hasPersistent")) {  //TODO
                ...
                synchronized (tempObject) {
                    if (!tempObject.getBoolean("hasPersistent")) {
                        try {
                            String hostName = tempObject.getString("hostName");
                            String ip = tempObject.getString("ip");
                            Timestamp lastUpdateTime = tempObject.getTimestamp("lastUpdateTime");
                            double cpuUsage = tempObject.getDoubleValue("cpuUsage"); // 会自动处理 null，默认返回 0.0
                            double netReceiveSpeed = tempObject.getDoubleValue("netReceiveSpeed");
                            double netSendSpeed = tempObject.getDoubleValue("netSendSpeed");

                            dispersedMapper.insertNewRecord(
                                    hostName,
                                    ip,
                                    lastUpdateTime,
                                    memUsage,
                            ...
                            logger.error("解析 JSON 或类型转换失败: {}", e.getMessage(), e);
                        } catch (org.apache.ibatis.exceptions.PersistenceException e) {
                            logger.error("数据库插入失败: {}", e.getMessage(), e);
                        } catch (Exception e) {
                            logger.error("未知错误发生: ", e);
                        }
                    }
                }
    ...
    public HybridDataCollectorService() {
        ...
        if (sampleSelect == 1) {
            ...
            for (HostConfigData hostConfigData : sshHostList) {
                logger.info("hostConfigData1 {}", hostConfigData);
                JSONObject initObject = dataSampleManager.sampleHostHardwareData(hostConfigData);
//                logger.info(String.format("initObject: %s", initObject));
                sshSampleData.put(hostConfigData.ip, initObject);
            }
...
//        float[][] loadCount = new float[][]{{1, 2, 3}, {3, 2, 1}, {4, 5, 6}};
        ...
        for (Map.Entry<String, JSONObject> hostInfo : hostsSampleData.entrySet()) {
            ...
            System.out.println("hostInfoJson: " + hostInfoJson);
